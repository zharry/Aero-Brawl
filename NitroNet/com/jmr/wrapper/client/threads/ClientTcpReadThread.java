...
/**
 * Networking Library ClientTcpReadThread.java Purpose: Waits for new incoming
...
public class ClientTcpReadThread implements Runnable {
	 ...
	 *            Instance of the server connection.
	 */
	...
	public void run() {
		try {
			...
			while (!serverConnection.getSocket().isClosed() && in != null) {
				...
				try {
					in.readFully(data);
				} catch (Exception e) { // Connection lost to server and didnt
										// finish sending data
					serverConnection.close();
					client.executeThread(
							new DisconnectedThread((SocketListener) client.getListener(), serverConnection));
					return; // kill thread
				}
				...
				if (objectArray[0] == 99) { // Complex entity
					...
					/**
					 * Check if the checksums are equal. If they aren't it means
					 * the packet was edited or didn't send completely.
					 */
